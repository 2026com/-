 ...
 *
 * 拆分说明（只移动代码位置，不改业务逻辑）：
 * - 网络请求/超时/错误格式化 → ./ai/network.js
 * - 输出 content 提取 / JSON 校验与截断修复 → ./ai/streamParser.js
 * - 本文件保留原有导出签名（chatCompletion / chatCompletionJSON），所有既有引用零改动
 */
import { chatCompletion, DEFAULT_TIMEOUT } from './ai/network.js'
import { parseModelJSON } from './ai/streamParser.js'
